...
import fs from "fs";
const config = JSON.parse(fs.readFileSync("firebase_options.json", "utf8"));
...
import {connectAuthEmulator, getAuth, sendEmailVerification, signInWithCustomToken} from "firebase/auth";
...
exports.addUser = functionAuth.user().onCreate(async (user, context) => {
...
exports.beforeUserCreated = functionAuth.user().beforeCreate(async (user, context):Promise<any> => {
  if (!(user && user.email)) {
...
exports.beforeSignIn = functionAuth.user().beforeSignIn((user, _context) => {
  debug(_context);
...
exports.verifyIdToken = function(idToken: string) {
